    ...
    mutate(buf) {
        ...
        for (let i = 0; i < nm; i++) {
            ...
            if (x === 0) {
                ...
                if (this.randBool()) {
                    res.writeUInt16BE(math_1.uint16(res.readUInt16BE(pos) + v), pos);
                }
                else {
                    res.writeUInt16LE(math_1.uint16(res.readUInt16LE(pos) + v), pos);
                }
            }
            else if (x === 9) {
                ...
                if (this.randBool()) {
                    res.writeUInt32BE(math_1.uint32(res.readUInt32BE(pos) + v), pos);
                }
                else {
                    res.writeUInt32LE(math_1.uint32(res.readUInt32LE(pos) + v), pos);
                }
            }
            else if (x === 10) {
